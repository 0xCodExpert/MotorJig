...
***************************************************************************************************/
__IO uint16_t cap_first = 0;
__IO uint32_t cap_freq = 0;
__IO uint32_t cap_duty = 0;

__IO uint32_t cap_1_2 = 0;
__IO uint32_t cap_1_3 = 0;

__IO uint16_t cap_1 = 0;
__IO uint16_t cap_2 = 0;
__IO uint16_t cap_3 = 0;
...
//			 printf("PWM2\r\n");
	...
	if (htim->Instance == TIM3)
	{
		...
		{
			//Falling Edge && After cap_1 is captured
			if(cap_first == 1)
			{
...
void Timer_InitTasks(void)
{
	xTimer_Uart = xTimerCreate("UartTimeOut",	/* Just a text name, not used by the RTOS kernel. */
				       RX1_TIMEOUT,				/* The timer period in ticks, must be greater than 0. */
					   pdFALSE,					/* The timers will auto-reload themselves when they expire.
					    						pdTRUE => Auto-Reload,  pdFALSE => One Time	*/
					   (void*)TMR_ID_UART, 		/* The ID is used to store a count of the
					          	  	  	  	  	number of times the timer has expired, which is initialised to 0. */
					   Timer_TimeOut); 			/* Each timer calls the same callback when it expires. */
